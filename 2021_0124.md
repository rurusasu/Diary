...
前述の `Dockerfile` でコンテナを構築すると，立ち上げ時に SHELL が立ち上げ時の log を表示したままとなるため，コマンドを入力することができない.





しかし，この `DockerImage` では `conda` コマンドを実行することができなかった．
...
この原因は，最後の `RUN` の PATH の一部が間違っていたから．
以下のように書き直すことで，`conda`コマンドを実行できるようになった．
