...
`JupyterNotebook` で開発している場合，[ノートブックで TensorBoard を使用する](https://www.tensorflow.org/tensorboard/tensorboard_in_notebooks?hl=ja) に記載されているように，`magic` を使用することで Notebook上に直接 `TensorBoard` を表示することができる．
...
そこで，次のようなコマンドを `JupyterNotebook` 上で事前に読み込んでおかなければならない可能性が浮上した『[学習中モデルをJupyter Notebook上のTensorBordでモニタリングする](https://qiita.com/msrohkwr/items/552af22406ebd9a3e8fa)』．
...
また，先ほどのコマンドは2回目を実行するとエラーを吐き，削除しても `TensorBoard` が実行出来なくなることはなかったので削除した．結果として，今回は直接表示することをあきらめ Web からアクセスする方法を取ることにする．

【4月7日 追記】: 後日 `Notebook` を再起動したところ，以下のようなエラーが発生した．

![TensorBoard Error1](https://raw.githubusercontent.com/rurusasu/Diary/master/%E7%94%BB%E5%83%8F/2021_0406/TensorBoard_Image_Err1.png)
...
ここまでの経験をもとに，UNetの訓練を `TensorBoard` 上で表示できるよう改良する．方法は非常に簡単で，`main` の最初に
