...
その結果、しっかりと `nvidia-graphics-drivers.conf` が削除されていた. この結果から、次の2つのことが考えられる.
...
[Ubuntu LinuxインストールNvidiaドライバー(最新のプロプライエタリドライバー)](https://www.cyberciti.biz/faq/ubuntu-linux-install-nvidia-driver-latest-proprietary-driver/#verification) に、 PCに搭載されている GPU の確認方法からドライバのインストール方法までが詳しく説明されていた. そこで、この記事を参考にインストールを行う.
